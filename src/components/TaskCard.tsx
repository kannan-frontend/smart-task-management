...
import { Pencil, Trash2, Calendar, User, Flag, CheckCheck, ClipboardEdit } from "lucide-react";
...
export default function TaskCard({ task, role, adminUser, users = [], onUpdate, onDelete }: TaskCardProps) {
        ...
        <div className="p-5 flex flex-col gap-3 flex-1">
          ...
          <div className="text-[11px] text-gray-400 dark:text-gray-500 space-y-1.5 border-t border-gray-50 dark:border-gray-700/50 pt-2">
            ...
            {(task.startDate || task.endDate) && (
              <div className="flex items-center gap-1.5">
                <Calendar size={10} />
                <span>{task.startDate}{task.startDate && task.endDate && " → "}{task.endDate}</span>
              </div>
            )}
            {task.remarks && (
              <div className="mt-1 px-2.5 py-1.5 bg-gray-50 dark:bg-gray-700/50 rounded-lg italic text-gray-500 dark:text-gray-400">
                "{task.remarks}"
              </div>
            )}
          </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {!confirmDelete ? (
                  ...
                  <div className="flex-1 flex items-center gap-1.5">
                    ...
                    <button onClick={async () => { await onDelete(task.id!); setConfirmDelete(false); }}
                      className="text-xs px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded-lg transition">Yes</button>
                    <button onClick={() => setConfirmDelete(false)}
                      className="text-xs px-3 py-1.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-lg transition">No</button>
                  </div>
                )}
              </div>
            </div>
