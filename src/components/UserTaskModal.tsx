...
import { X, ChevronDown, Calendar, StickyNote, ListTodo } from "lucide-react";
...
export default function UserTaskModal({ isOpen, onClose, task, onUpdate }: Props) {
  const isDone = task.status === "done";

  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } =
    useForm<UserTaskFormValues>({
      resolver: yupResolver(userTaskSchema),
      defaultValues: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" aria-modal="true">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md z-10 overflow-hidden">
        ...
        {/* Header */}
        <div className="px-6 pt-6 pb-4 border-b border-gray-100 dark:border-gray-700 flex items-start justify-between gap-3">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <select {...register("status")} disabled={isDone}
          ...
          {/* Remarks */}
          <div>
            <label className={lbl}><StickyNote size={13} /> Remarks</label>
            <textarea {...register("remarks")} rows={3} disabled={isDone}
